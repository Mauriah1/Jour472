...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer: there are 24 rows, but there is only 23 counties.**

```{r}
maryland_population_with_voters <- maryland_population |> left_join(county_voters_2022, join_by("GE"))
```
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer: This data contains information that shows the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county.**
