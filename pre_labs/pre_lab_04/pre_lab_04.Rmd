...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with Y. Name a problem that you see that is preventing proper grouping. **Answer: There are multiple versions of the same Grantee, since they have different capitialization the computer is storing different grantee's when they should be together.**
...
Run the following code to make a copy of the Grantee field called grantee_clean. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer: It combined all the names in the data that had the same name but different capitalization all together. It also changed the grantee clean version to the most popular name that was used within the data.**
...
Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with Y. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer: No, because the differences are different enough.**
