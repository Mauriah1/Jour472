...
**Answer** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url. <https://datawrapper.dwcdn.net/npna2/1/>
...
md_gov_county <- md_gov_county |> mutate(moore_pct = moore/(cox + moore + lashar + wallace + harding)) 
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer** Describe the changes from the previous map: It zoomed in on the state of Maryland.
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "0.0%")}} **Answer** Describe the changes from the previous map: The changes from the first map is that this map highlights what county is which.
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **Answer** Put the URL of your map here. <https://datawrapper.dwcdn.net/55juM/1/>
