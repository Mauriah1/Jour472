...
>>>>>>> d921576f3617ece8968915835bb71a7725c00f17
...
<<<<<<< HEAD
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: The "Vendor Name" and "Date" field is a character data type. The "Amount" field is a dbl data type.**
=======
...
<<<<<<< HEAD
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? **Answer There are 549 zip codes that are less than five characters, I think that because we don't have any other geographical information that would tell us if the data is missing a leading zero or some other character.**
=======
...
<<<<<<< HEAD
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: That there are na's, which are unusual values in a dataset.**
=======
...
<<<<<<< HEAD
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: The Department of Natural Resources/Rural Legacy Program fiscal year in 2009, had the lowest amount of grant and loans.**
=======
...
<<<<<<< HEAD
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer: It makes the capitalization the same across all the names and it changed zip code to zip_code.**
=======
...
<<<<<<< HEAD
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer: There are are 8 different categories that are duplicated, but there is 50 duplicates total.**
=======
...
<<<<<<< HEAD
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer: The new dataframe has a total of 17,740 rows.**
=======
...
<<<<<<< HEAD
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: The difference is that it removed any other numbers beyond the 5 digits within a zip code.**
=======
...
<<<<<<< HEAD
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Does that zip code make sense to you? **21202 has gotten the most money from the Maryland Tourism Board.**
