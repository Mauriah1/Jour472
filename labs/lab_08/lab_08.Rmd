...
```{r}
PG_County_911_calls <- read.csv("data/prince_georges_2022_overdoses.csv")

PG_County_911_calls <- PG_County_911_calls |> 
...
```{r}
PG_County_911_calls_by_date <- PG_County_911_calls |>
  group_by(month) |>
  summarise(total_calls=n()/1397 *100)|>
  arrange(desc(month))
```

```{r}
PG_County_911_calls_by_date |>
  ggplot() +
  geom_bar(aes(x=reorder(month, total_calls), weight=total_calls)) +
  coord_flip() +
  labs(
    title = 'Prince George\'s County Overdose Calls',
    x = 'month',
    y = 'percentage of calls',
    caption = 'Souce: PG County EMS'
  )
```
...
```{r}
PG_County_911_calls_by_week <- PG_County_911_calls |>
  group_by(week) |>
  summarise(total_calls=n()/1397 *100)|>
  arrange(desc(week))
```
...
```{r}
pg_calls_zipcode <- PG_County_911_calls |> group_by(zipcode) |>
  summarise(total_calls = n())

md_median_income_zip <- get_acs(geography = 'zcta', variables = 'B19013_001', state = 'MD', year = 2019)

pg_calls_with_income <- pg_calls_zipcode |> inner_join(md_median_income_zip, by = c('zipcode' ='GEOID'))
