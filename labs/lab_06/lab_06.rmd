...
county_911_calls_combined <- bind_rows(list(allegany_911, carroll_911, cecil_911))
```
...
Then calculate the most calls per county and location. Which location had the most calls - where and what is that location? Cecil county, had 5 months that had 55 calls from the location 314 grove neck rd.
...
Which zip code has the highest rate of calls per 1,000 people? Find the neighborhoods in that zip code that are listed in the data - you can use group_by or distinct to do this. What are some of those neighborhoods? zipcode 21217 had the most calls, this is in the area
...
A4: What is the top locations zipcode that had a percentage over 50, for calls that came from homes that owned by homeowners?

```{r}
baltimore_911_with_demographics |>
  filter(pct_homeowner >= 50) |>
  group_by(zip_code, location) |>
  summarise(
    count=n()
  ) |>
  arrange(desc(count))
