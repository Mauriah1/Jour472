...
# make a list for orgs with most grants 
results |>
...
When you finish scraping into a dataframe, write code to find the press releases that have the word "police" in the title. How many are there and when was the most recent one? There are 9 total and 2021-06-21 is the most recent one.

**A2**

```{r}
releases_url <- "https://www.opd.state.md.us/press-releases" 

results_2 <- releases_url |> 
  read_html()

press_results <- results_2 |> html_elements('a.wixui-rich-text__text')|> html_text() |>
  as_tibble() |> separate(value, c('date', 'title'), sep=":") |> drop_na(title)|> mutate(title = str_squish(title), date = mdy(date))

 press_results |> filter(str_detect(title, 'Police'))
```
...
```{r}
press_releases_url <- "https://www.cardin.senate.gov/?post_type=press-releases"

results_3 <- press_releases_url |> 
  read_html()

press_results_2 <- results_3 |> html_elements('list-press-releases time')|> html_text() |>
  as_tibble() |> separate(value, c('date', 'title'), sep=":") |> drop_na(title)|> mutate(title = str_squish(title), date = mdy(date))
