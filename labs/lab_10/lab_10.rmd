...
**A1. PG County and Baltimore County shows the lowest difference.**

```{r}
#Calculate percentages for both candidates and the difference between them per county
md_voters <- read_csv("data/md_statewide_county.csv") 
...
glimpse(md_voters)
 ...
 md_voters <- md_voters |>
   mutate(total_gov = cox + moore + lashar + wallace + harding + sefcik + gov_writein, 
          total_og = peroutka + brown + ag_writein) |> 
   
   mutate(pct_moore = (moore/total_gov) *100, 
          pct_brown = (brown/total_og) *100, 
...
md_voters <- md_voters |> mutate(geoid= as.character(geoid))
```

```{r}
#Merge geometries with voting data
 md_votes_geo <- md_counties |> inner_join(md_voters, by = "geoid")
```

```{r}
#Create the map
ggplot() + geom_sf(data = md_votes_geo, aes(fill=pct_difference))
```
...
md_outages <- read_csv("data/county_outages_march2023.csv") |> mutate(rate = outage_customers/total_customers*10000)
...
md_county_outages_geo <- md_counties_fixed |> inner_join(md_outages, by=c('NAME'='county'))
...
electric_hybrid_with_geo <- md_counties_fixed |> inner_join(electric_hybrid, by=c('NAME'='county'))|> clean_names()
```
...
md_county_population <- get_acs(geography = "county",
              ...
              year = 2021,
              state = "MD") |> clean_names()
```

```{r}
#Calculate per capita rate for March 2023
electric_hybrid_with_geo <- electric_hybrid_with_geo |> inner_join(md_county_population, by=('geoid')) |> mutate(rate = march_2023/estimate*10000)
```

```{r}
#Map out the per capita rate
ggplot() +
  geom_sf(data=electric_hybrid_with_geo, aes(fill=rate)) +
  theme_minimal() +
  scale_fill_viridis_b(option="magma", trans='log')
```
